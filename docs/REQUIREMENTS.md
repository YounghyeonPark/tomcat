...
| **NFR15** | **Disturbance cases the robot MUST survive**  | a **15 N / 0.1 s push** (48 mm), an **unexpected 40 mm step**, and a **10° lateral slope**. A 30 N shove (96 mm) is explicitly OUT of scope. Met with ~19 % margin **in the reduced-order model** (ADR-0017). `[assumed]` scenarios. ⚠️ **NOT demonstrated in simulation:** closed-loop MuJoCo reaches **22.5 mm** in its worst direction against the 48 mm required and 52.7 mm predicted — a **2.3× shortfall** (ADR-0027). The prediction assumes optimal control on a single axis; the measurement uses a proportional law and sweeps direction, so the gap cannot yet be attributed to either. |
